...
const FeeCategoryManager = () => {
  ...
  const { data: categories = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('fee_categories').select('*').order('sort_order');
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editId) {
        const { error } = await supabase.from('fee_categories').update(payload).eq('id', editId);
        if (error) throw error;
      } else {
        const maxSort = categories.length > 0 ? Math.max(...categories.map((c: any) => c.sort_order || 0)) : 0;
        const { error } = await supabase.from('fee_categories').insert({ ...payload, sort_order: maxSort + 1 });
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('fee_categories').update({ is_active: false }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['fee_categories'] });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-display font-bold text-foreground">{bn ? 'ফি ক্যাটাগরি ব্যবস্থাপনা' : 'Fee Category Management'}</h3>
