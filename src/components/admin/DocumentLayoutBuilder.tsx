...
const DocumentLayoutBuilder = () => {
  ...
  const handlePrint = () => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>${formName}</title><style>@font-face{font-family:"SutonnyOMJ";src:url("/fonts/SutonnyOMJ.ttf") format("truetype");font-display:swap;}</style><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+Bengali:wght@400;600;700&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'SutonnyOMJ','Noto Sans Bengali',sans-serif;padding:${ps.marginTop}mm ${ps.marginRight}mm ${ps.marginBottom}mm ${ps.marginLeft}mm;font-size:12px}table{width:100%;border-collapse:collapse}td,th{border:1px solid #333;padding:6px 8px;text-align:left}.footer-area{margin-top:32px;display:flex;justify-content:space-between}${paperCss}@media print{@page{margin:${ps.marginTop}mm ${ps.marginRight}mm ${ps.marginBottom}mm ${ps.marginLeft}mm}}</style></head><body>${el.innerHTML}</body></html>`);
    w.document.close();
